...
async function handler(req, res) {
    ...
    try {
        ...
        if ((fromUser.limit || 0) < parsedAmount) {
            return res.status(400).json({
                message: 'Insufficient credit limit for transfer',
                currentLimit: fromUser.limit || 0,
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            const updatedFromUser = await tx.user.update({
                where: { id: fromUserId },
                data: { limit: ((fromUser.limit || 0) - parsedAmount) }
            });
            // Add to destination user
            const updatedToUser = await tx.user.update({
                where: { id: toUserId },
                data: { limit: ((toUser.limit || 0) + parsedAmount) }
            });
        ...
        return res.status(200).json({
            ...
            data: {
                fromUser: {
                    id: fromUserId,
                    oldLimit: fromUser.limit || 0,
                    newLimit: result.updatedFromUser.limit,
                    change: -parsedAmount
                },
                toUser: {
                    id: toUserId,
                    oldLimit: toUser.limit || 0,
                    newLimit: result.updatedToUser.limit,
                    change: parsedAmount
                },
                amount: parsedAmount,
                remark
