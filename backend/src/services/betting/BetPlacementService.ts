...
import { betCategorizationService } from "./BetCategorizationService";
import logger from "../../monitoring/logging/logger";

const prisma = new PrismaClient();

// Abstract base class for bet handlers
abstract class BetHandler {
...
class MatchBetHandler extends BetHandler {
  async handle(bet: Bet): Promise<BetPlacementResult> {
    try {
      ...
      logger.info(`Match bet placed successfully: ${matchBet.id}`, {
        userId: bet.userId,
        marketName: bet.marketName,
        odds: bet.odds,
        stake: bet.stake
      ...
      logger.error("Failed to place match bet:", error);
      return {
        success: false,
        betType: "match",
...
class SessionBetHandler extends BetHandler {
  async handle(bet: Bet): Promise<BetPlacementResult> {
    try {
      ...
      logger.info(`Session bet placed successfully: ${sessionBet.id}`, {
        userId: bet.userId,
        marketName: bet.marketName,
        odds: bet.odds,
        stake: bet.stake
      ...
      logger.error("Failed to place session bet:", error);
      return {
        success: false,
        betType: "session",
...
export class BetPlacementService {
  private handlers: Map<BetType, BetHandler>;
  ...
  registerHandler(handler: BetHandler): void {
    ...
    logger.info(`Registered bet handler for type: ${handler.getType()}`);
  }
   ...
   */
  async placeBet(bet: Bet): Promise<BetPlacementResult> {
    try {
      // Categorize the bet
      const betType = betCategorizationService.categorizeBet(bet);
      ...
      if (!handler) {
        logger.error(`No handler found for bet type: ${betType}`);
        return {
          success: false,
          betType,
      ...
      logger.error("Error in bet placement service:", error);
      return {
        success: false,
        betType: bet.betType || "match",
   ...
   */
  private validateBet(bet: Bet): { valid: boolean; error?: string } {
   ...
   */
  async getBetStatistics(userId: string, betType?: BetType): Promise<any> {
    try {
      const stats: any = {};
...
export async function placeBet(bet: Bet): Promise<BetPlacementResult> {
  return betPlacementService.placeBet(bet);
}
