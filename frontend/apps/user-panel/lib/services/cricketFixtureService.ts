...
export class CricketFixtureService {
  ...
  /**
   * Get live fixtures (alias for getLiveMatches)
   */
   ...
   */
  static async getFixturesByStatus(status: string) {
    return await prisma.match.findMany({
      where: {
  ...
  /**
   * Get active fixtures (alias for getActiveMatches)
   */
  static async getActiveFixtures() {
    return await this.getActiveMatches();
  }

  /**
   * Bulk upsert fixtures
   */
  static async bulkUpsertFixtures(fixtures: FixtureData[]) {
    return await this.saveMatchesFromFixtures(fixtures);
  }
