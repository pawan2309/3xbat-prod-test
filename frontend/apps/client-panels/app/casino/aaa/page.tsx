...
export default function AAAPage() {
  ...
  const [gameData, setGameData] = useState<AAAData | null>(null)
  const [countdown, setCountdown] = useState(0)
  const [isConnected, setIsConnected] = useState(false)
  const [lastUpdate, setLastUpdate] = useState<string>('')

  useEffect(() => {
    // Connect to WebSocket for AAA game
    const connectToGame = async () => {
      try {
        await websocketManager.joinCasinoGame('aaa')
        setIsConnected(true)
        console.log('Connected to AAA game')
      } catch (error) {
        console.error('Failed to connect to AAA game:', error)
      }
    }
  ...
  const getCardColor = (cardValue: string) => {
    ...
    // AAA specific card colors - Three Aces
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-pink-900 to-red-900 text-white">
      ...
      <div className="bg-black bg-opacity-50 p-4">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-2xl font-bold text-center flex-1">AAA - Three Aces</h1>
      ...
      <div className="p-4 text-center">
        ...
        {/* Countdown Timer */}
        <div className="bg-gradient-to-r from-pink-600 to-purple-800 rounded-lg p-6 mb-6">
      ...
      <div className="px-4 pb-8">
        <div className="bg-black bg-opacity-30 rounded-lg p-6">
          ...
          <div className="space-y-8">
            ...
            <div className="text-center">
              <h4 className="text-lg font-semibold mb-4 text-yellow-400">🃏 ACES</h4>
              <div className="flex justify-center space-x-4">
                {gameData ? [
                  gameData.C1, gameData.C2, gameData.C3
                ].map((card, index) => (
                  <div key={index} className={`w-20 h-24 rounded-lg border-2 flex items-center justify-center ${getCardColor(card)}`}>
                    <img 
                      src={getCardImage(card)} 
                      alt={`Ace Card ${index + 1}`}
                      className="w-16 h-20 object-contain"
                    />
                  </div>
                )) : Array.from({ length: 3 }, (_, index) => (
                  <div key={index} className="w-20 h-24 rounded-lg border-2 bg-gray-200 flex items-center justify-center">
                    <img 
                      src="/cards/back.svg" 
                      alt="Card back"
                      className="w-16 h-20 object-contain"
            ...
            {/* Second Row - Red Cards */}
            <div className="text-center">
              <h4 className="text-lg font-semibold mb-4 text-red-400">🔴 RED CARDS</h4>
              <div className="grid grid-cols-5 gap-2 justify-center">
                {gameData ? [
                  gameData.C4, gameData.C5, gameData.C6, gameData.C7, gameData.C8,
                  gameData.C9, gameData.C10
                ].map((card, index) => (
        ...
        <div className="mt-6 bg-black bg-opacity-30 rounded-lg p-4">
          <h4 className="text-lg font-semibold mb-3">AAA Game Rules</h4>
          <div className="text-sm space-y-2">
            <p>• <span className="text-yellow-400">Aces</span>: Cards 1-3 (Special cards)</p>
            <p>• <span className="text-red-400">Red Cards</span>: Cards 4-10 (Red side)</p>
            <p>• <span className="text-blue-400">Blue Cards</span>: Cards 11-20 (Blue side)</p>
            <p>• Bet on which group will have the winning card</p>
            <p>• Aces have special winning conditions</p>
