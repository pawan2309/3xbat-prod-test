...
export default function DT20Page() {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      setSocket(newSocket)
      // Join casino room
      newSocket.emit('join_casino_room', { game: 'dt20' })
    })

    newSocket.on('casino_tv_updated', (payload: any) => {
      if (payload.game === 'dt20' && payload.data) {
        console.log('🎰 Received casino TV data:', payload)
        // Extract stream URL from the data - use the actual stream URL from jmdapi
        if (payload.data && payload.data.data) {
          // The data comes from jmdapi.com/tablevideo/?id=3035
          setCasinoTvUrl(`https://jmdapi.com/tablevideo/?id=3035`)
        }
      }
    })
  ...
  useEffect(() => {
    ...
    console.log('🎰 Casino data will come via WebSocket for: dt20')
